...
fun main() {
    ...
    var coffeeList = mutableListOf<Menu>()                                // 데이터 클래스에 있는 프로퍼티(타입 자리)로 이뤄진 커피 리스트.
    for(coffee in coffeeData) {                                          // 딕셔너리를 감싸는 리스트의 인덱스 순서대로 딕셔너리가 하나씩 coffee에 담겨 데이터 클래스의 프로퍼티로 생성돼 새로 만든 가변 리스트에 추가됨.
    ...
    var latteList = mutableListOf<Menu>()                               // 데이터 클래스에 있는 프로퍼티로 이뤄진 라떼 리스트.
    for(latte in latteData) {
        var latteId = latte.get("id").toString().toInt()
    ...
    var teaList = mutableListOf<Menu>()                                 // 데이터 클래스에 있는 프로퍼티로 이뤄진 티 리스트.
    for(tea in teaData) {
        var teaId = tea.get("id").toString().toInt()
    ...
    //    println(coffeeList)  // 제이슨 형식으로 미리 입력해놓은 데이터를 메뉴 데이터 클래스를 이용해 데이터 클래스 객체 생성(커피 가변 리스트 객체, 라떼 가변 리스트 객체, 티 가변 리스트 객체)
    ...
    // 카테고리 번호와 해당 음료 번호 추출(Pair 사용으로 값 2개 반환)
...
//    println(drinkCategoryId_and_drinkNumber.toString().get(4)) // 3 -> 해당 음료번호
    ...
    var orders = Orders()
    if (category == 1) {
        var yourDrink = coffeeList.get(drinkId - 1)    // drinkId를 인덱스로 사용해 해당 카테고리 메뉴 리스트에 접근해서 인데스보다 size가 1 크기 때문에 -1
